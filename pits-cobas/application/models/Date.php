...
class Pits_Model_Date
{
    ...
    /**
     * Construit un objet Pits_Model_Date
     ...
     *
     * Un contrôle de validité est réalisé et à la construction de Pits_Model_Date mais devra être demandé
    ...
    public function __construct($maDate='', $locale='', $sepdate='', $septime=':', $time=false)
    {
        if (!empty($locale) && $locale != 'fr_FR') {
            throw new Pits_Model_Exception('Cette locale n\'est pas gérée par Pits_Model_Date');
        } elseif (empty($septime)) {
            throw new Pits_Model_Exception('Le séparateur de temps ne peut pas être vide dans Pits_Model_Date');
        } elseif ($septime == ' ' || $sepdate == ' ') {
            throw new Pits_Model_Exception('Les séparateurs de date et temps ne peuvent pas être un espace dans Pits_Model_Date');
        }
        $this->_timeOnly = $time;
        if ($maDate == '') {
    ...
    {
        if ($this->_error) throw new Pits_Model_Exception($this->_errorMessage);
        try {
            ...
            throw new Pits_Model_Exception("Erreur de format d\'une date : $format !");
        }
    }
}