...
abstract class Tp_Document_Abstract
{
    /**
    ...
    public function __construct($filename)
    {
        ...
        if (!$this->_file) {
            throw new Tp_Document_Exception("Le ficher `$filename` modèle du document n'est pas trouvé dans le répertoire `application/configs/doc`.");
        }
    }
    ...
    {
        if (!is_string($field)) {
            throw new Tp_Document_Exception("Le nom champ doit être de type `string`.");
        }
        $field = '#§' . $field . '§#';
        ...
        if ($i == count($this->_tableFields)) {
            throw new Tp_Document_Exception("Le champ `$field` de l'instruction §liste()§ est inconnu.");
        }
    ...
    private function _dateFR()
    {
        if (preg_match('#§dateFR\((.+)\)§#i', $this->_buffer, $matches) == 1) {
            $t = explode(',', $matches[1]);
            if (count($t) != 2) {
                throw new Tp_Document_Exception("Erreur de synthaxe dans l'instruction §dateFR(format,champDate)§.");
            }
            try {
                ...
                if (substr($laDate, 0, 10) == '0000-00-00') {
                    ...
                    Tp_Model_Format::date($t[0], $laDate),
                    $this->_buffer);
                }
            } catch (Exception $e) {
                $this->_buffer = preg_replace('#§dateFR\((.+)\)§#i',
                Tp_Model_Format::date($t[0], $this->getField($t[1]), 'fr_FR'),
                $this->_buffer);
            }
        }
    }
        ...
        if ($tmp == '§endif§') {
            ...
            if (preg_match('#^\((.+)\)$#i', $tmp, $matches) == 1) {
                ...
                if (isset($res[0])) {
                    ...
                    throw new Tp_Document_Exception("\nErreur de lecture du champ `$tmp` dans la méthode getFields() de l'objet Tp_Document_Abstract.\n");
                }
