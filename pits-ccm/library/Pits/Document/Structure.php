 ...
 * @author     pomirol
 */
abstract class Pits_Document_Structure
{
    /**
    ...
    public function __construct($filename)
    {
        ...
        $this->_file = fopen('config' . DIRECTORY_SEPARATOR . $filename, 'r', true);
        if (!$this->_file) {
            throw new Pits_Document_Exception("Le ficher `$filename` modèle du document n'est pas trouvé dans le répertoire `application/config/`.");
        }
    }
    ...
    private function _remarque()
    {
        ...
        $this->_buffer = Pits_Document_Structure::mb_trim(preg_replace('#§rem:(.+)§#i','',$this->_buffer));
        ...
        if (preg_match('#§dateFR\((.+)\)§#i', $this->_buffer, $matches) == 1) {
            ...
            try {
                ...
                if (substr($laDate, 0, 10) == '0000-00-00') {
                    ...
                    Pits_Format::date($t[0], $laDate),
                    $this->_buffer);
                }
            } catch (Exception $e) {
                $this->_buffer = preg_replace('#§dateFR\((.+)\)§#i',
                Pits_Format::date($t[0], $this->getField($t[1]), 'fr_FR'),
                $this->_buffer);
            }
        }
    }
    ...
    private function _conditionnelle()
    {
        ...
        if ($tmp == '§endif§') {
            ...
            if (preg_match('#^\((.+)\)$#i', $tmp, $matches) == 1) {
                ...
                $tmp = $this->mb_trim($matches[1]);
                if (preg_match('#^(.+)==(.+)#i', $tmp, $matches) == 1) {
                    $res = $this->getFields(array($this->mb_trim($matches[1]), $this->mb_trim($matches[2])));
                    if (isset($res[0]) && isset($res[1])) {
                        $this->_condition = ($res[0] == $res[1]);
                    } else {
                        throw new Pits_Document_Exception("\nErreur de lecture du la partie `$tmp` dans la méthode " . __METHOD__ . " de l'objet Pits_Document_Structure.\n");
                    }
                } else {
