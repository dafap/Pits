...
class Pits_Validate_Mdp extends Zend_Validate_Abstract
{
     ...
     * @param int min (nombre minimum de caractères - 0 par défaut)
     * @param int max (nombre maximum de caractères - null par défaut donc illimité)
     * @param int nbDigits (nombre minimum de chiffres - 1 par défaut)
     * @param string mdp (vide ou mdpIni à changer - vide par défaut)
     */
    public function __construct($min=0, $max=null, $nbDigits=1, $mdpIni='')
    {
        $this->min = $min;
        $this->max = $max;
