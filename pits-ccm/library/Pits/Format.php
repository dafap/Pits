...
class Pits_Format
{
    ...
    public static function prenom($data)
    {
        if (is_string($data)) {
            ...
            throw new Pits_Format_Exception(self::NOT_STRING);
        }
    }
    /**
     ...
     * 
     * @param string $format (voir class Pits_Date)
     * @param string $maDate='' (si vide, retourne la date courante) 
     * @param string $locale (voir class Pits_Date)
     * @return string
     */
    public static function date($format, $maDate='', $locale='') {
        if (!empty($maDate)) {
            if (empty($locale)) {
                $pits_date = new Pits_Date($maDate);
            } else {
                $pits_date = new Pits_Date($maDate, $locale);
            }
        } else {
