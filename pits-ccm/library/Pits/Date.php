...
class Pits_Date
{
     ...
     * Un contrôle de validité est réalisé et à la construction de Pits_Date mais devra être demandé
     ...
     */
    public function __construct($maDate='', $locale='', $sepdate='', $septime=':')
    {
        if (!empty($locale) && $locale != 'fr_FR') {
            throw new Pits_Date_Exception('Cette locale n\'est pas gérée par Pits_Date');
        } elseif (empty($septime)) {
            throw new Pits_Date_Exception('Le séparateur de temps ne peut pas être vide dans Pits_Date');
        } elseif ($septime == ' ' || $sepdate == ' ') {
            throw new Pits_Date_Exception('Les séparateurs de date et temps ne peuvent pas être un espace dans Pits_Date');
        }
        if ($maDate == '') {
    ...
    {
        if ($this->_error) throw new Pits_Date_Exception($this->_errorMessage);
        try {
            ...
            throw new Pits_Date_Exception("Erreur de format d\'une date : $format !");
        }
    }
}