 ...
 * @author     pomirol
 */
class PdfEleve extends Pits_Document_Structure
{
    /**
     * Passerelle utilisée
    ...
    public function __construct($filename, $eleveId){
        ...
        $this->_tEleveslist = new TEleveslist();
    }
    public function run($p, $y, $margeG)
    {
        ...
        while (!$this->feof()) {
            $this->_buffer = Pits_Document_Structure::mb_trim($this->fgets(), '\\\\s', false);
    ...
    protected function particularValues()
    {
        ...
        if (preg_match('#§regime§#i', $this->_buffer, $matches) == 1) {
            ...
            $this->_buffer = preg_replace('#§regime§#i', $regime[0] ? 'interne' : 'demi-pensionnaire', $this->_buffer);
        }
    }
}